...
% Initialize video writer for capturing frames
fig = figure;
M(nsteps) = struct('cdata', [], 'colormap', []);

fid = fopen('DebyeADE.txt', 'w');

% Initialize material parameters for the Debye medium
for k = ks:ke - 1
    ga(k) = 1 / (epsr + (sigma * dt / epsz) + (chi1 * dt / tau));
    gb(k) = sigma * dt / epsz;
    gc(k) = (chi1 * dt / tau) / (1 + (dt / (2 * tau)));
end
...
for t = 1:nsteps
    ...
    % Update Ex field using ADE method for Debye medium
    for k = 2:ke - 1
        ex(k) = ga(k) * (dx(k) - ix(k) - sx(k));
        ix(k) = ix(k) + gb(k) * ex(k);
        sx(k) = del * sx(k) + gc(k) * ex(k);
    end
    ...
    % Capture and store frames every 5 steps for smoother video
    if mod(t, 5) == 0
        plot(1:ke, ex, 'LineWidth', 2);
        grid on;
        axis([0 ke -1 1.2]);

        titlestring = 'Gaussian Pulse, ADE method';
