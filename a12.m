...
for t = 1:nsteps
    ...
    % Update Ex field using Z method in the plasma region
    for k = 2:ke
        if (k >= kc && k <= kend)
            omega = 2 * pi * 1e12;
            vc = 0.01e12;
            ex(k) = (dx(k) - sx(k));
            sx(k) = (1 + exp(-vc * dt)) * sx1(k) - exp(-vc * dt) * sx2(k) + ((omega^2) * dt / vc) * (1 - exp(-vc * dt)) * ex(k);
            sx2(k) = sx1(k);
            sx1(k) = sx(k);
        else
            ex(k) = dx(k);
    ...
    if mod(t, 10) == 0
        ...
        titlestring = 'Frequency of propagating wave = 0.7 THz, Z method';
