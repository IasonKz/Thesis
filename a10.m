...
for t = 1:nsteps
    ...
    % Update Ex field in the plasma region
    for k = 2:ke
        if (k >= kc && k <= kend)
            omega = 2 * pi * 1e12;
            vc = 0.01e12;
            ex(k) = (dx(k) - ix(k) - exp(-vc * dt) * sx(k));
            ix(k) = ix(k) + ((omega^2) * dt / vc) * ex(k);
            sx(k) = exp(-vc * dt) * sx(k) + (-(omega^2) * dt / vc) * ex(k);
        else
            ex(k) = dx(k);
        end
    end
    ...
    if mod(t, 10) == 0
        ...
        titlestring = 'Frequency of propagating wave = 1 THz';
        ...
        text(320, 1.05, 'Plasma', 'FontSize', 16, 'Color', 'g');
        text(320, 0.9, 'freq = 1 THz', 'FontSize', 16, 'Color', 'g');
