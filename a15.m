...
for t = 1:nsteps
    ...
    % Update Ex field using ADE method in Lorentz medium
    for k = 2:ke
        if (k >= kc && k <= ke)
            omega = 2 * pi * 0.5e12;
            ex(k) = (1 / epsr) * (dx(k) - sx(k));
            
            sx(k) = ((2 - (dt * omega)^2) / (1 + dt * delta * omega)) * sx1(k) - ...
                    ((1 - dt * delta * omega) / (1 + dt * delta * omega)) * sx2(k) + ...
                    ((dt^2 * omega^2 * eps1) / (1 + delta * omega * dt)) * ex(k);
    ...
    if mod(t, 10) == 0
        ...
        titlestring = 'Frequency of propagating wave = 3 THz (ADE)';
